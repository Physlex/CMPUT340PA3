...
%Start position
desired = [-0.2, -0.2, 0.5];
theta = invKin3D(ls, theta0', desired', n, mode);
...
% Define start and end pos for curr iter

for can_index = 1:n_cans
    % Get current can positions
    start_pos = start_can_pos(can_index, 1:3);
    end_pos = end_can_pos(can_index, 1:3);

    obj_picked = obj_picked + 1;
    for k = 1:steps
        % Interpolate between start and end using bezier for segment
        path = bezier(start_pos, end_pos, steps, k);

        % Find the angles that allow the render to conclude from segment
        theta = invKin3D(ls, theta, path', n, mode);

        % Render result of curr segment
        clf;
        hold on;
        plot_scene(obj_picked, obj_placed, start_can_pos, end_can_pos, gca, ls, theta');
    end
    obj_placed = obj_placed + 1;

    % Render result of current movement
    clf;
    hold on;
    plot_scene(obj_picked, obj_placed, start_can_pos, end_can_pos, gca, ls, theta');
end
...
%%%%%%%%%~~~~~END~~~~~~%%%%%%%%%%%%%%%%%%%
%When moving from table to conveyor, plot can at the end of arm
